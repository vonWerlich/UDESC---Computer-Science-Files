...
-- Trabalho Parte 3 Compiladores
-- Percorrer a árvore sintática gerada após análise semântica gerando um arquivo com o
-- assembly da JVM que posteriormente deverá ser montado pelo Jasmin 
...
--genCmd tab fun (Atrib id (Const (CInt 1) :+: IdVar varId)) | id == varId = do
--    (tl, pos) <- infoVar tab id
--    return (genInt 1 ++ genAtrib tl TInt pos)
--
...
comandoAtrib = do{reserved "++"; id <- indentifier; symbol ";"; return (Atrib id (Const (CInt 1) :+: IdVar id)) } 
...
genProg nome (Prog fun cfun tab b) = do let novaTab = definePos 0 tab -- Aplicando definePos na tabela de variáveis
                                        c <- genCab nome 
                                        mc <- genMainCab 4 4 
                                        b' <- genBloco novaTab fun b
                                        return (c++mc++b'++"\n.end method\n")
...
genCmd tab fun (If e b1 b2) = do {
    verdadeiro <- novoLabel;
    falso <- novoLabel;
    fora <- novoLabel;

    e' <- genExprL tab fun verdadeiro falso e;

    b1' <- genBloco tab fun b1;
    b2' <- genBloco tab fun b2;

    if b2' == [] then return(e' ++ verdadeiro ++ ":\n" ++ b1' ++ falso ++ ":\n")
else return(e' ++ verdadeiro ++ ":\n" ++ b1' ++ "\tgoto " ++ fora ++ "\n" ++ falso ++ ":\n" ++ b2' ++ fora ++ ":\n")

}
genCmd tab fun (Proc i es) =
    do{
        es' <- mapM (genExpr tab fun) es;
        return (concat (map snd es') ++ "\tinvokestatic Vitor/"++i++"("++genArgs (map fst es')++")"++tipoArg (tipoFun fun i) ++ "\n")
    }
...
genCmd tab fun _ = return ""
--genCmd tab fun (DoWhile exp bloc) = do {l <- novoLabel; l' <- novoLabel; l'' <- novoLabel; exp' <- genExprL tab fun l' l'' exp;
--                                           bloc' <- genBloco tab fun bloco;
--                                           return (l'++":\n"++bloco'++exp'++l''++":\n")}
...
genExpr tab fun (Const (CString s)) = return (TString, "\tldc " ++ "\"" ++ s ++ "\"" ++ "\n")
...
--genExpr tab fun t = return (TInt, show t)
...
main2 = do input <- readFile "texto2.txt"
           let sin = parserE input
           case sin of 
                Left er -> print er
                Right v -> printSemantica v

main = do input <- readFile "texto2.txt"
          let sin = parserE input
          case sin of 
                Left er -> print er
                Right v -> do
                       let output = evalState (genProg "nome" v) 0
                       putStrLn output