...
#define N 51000 //numero de iterações total 
#define T0 100
#define Tn 0.001
...
void gerarVizinho1(int *vizinho, const int *solucao) {
    ...
    int metodo = rand() % 3;  // Escolhe o tipo de vizinho
...
void gerarVizinho2(int *vizinho, const int *solucao) {
    ...
    int metodo = rand() % 4;  // Três métodos possíveis
...
int main () {
	...
	//"TSP_100.txt"
	const char *nomearquivo = "TSP_51.txt";
	read_file(nomearquivo);
	
	FILE *saida = fopen("saida_boxplot_1.txt", "a");
		if (saida == NULL) {
